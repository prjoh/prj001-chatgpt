...
struct MovementSystem {
    ...
    void update(float deltaTime) {
      ...
      for (auto& [entity, position] : *positions) {
        ...
        rotation.angle += (input.right - input.left) * 160.0f * deltaTime;
      ...
      for (auto& [entity, ai] : *ais) {
        ...
        if (distance <= ai.attackRange) {
          ...
          if (ai.attackCooldown <= 0) {
            ai.attack_time += deltaTime;

            ai.shoot_cooldown -= deltaTime;
            if (ai.shoot_cooldown <= 0) {
              ...
              projectile.x = position.x + render.spriteRect.w * 0.5 - 5;
              projectile.y = position.y + render.spriteRect.h * 0.5 - 5;
              projectile.velocityX = direction_x * 500.0;
              projectile.velocityY = direction_y * 500.0;
...
int main(int argc, char** argv) {
  ...
  soundPath += "res/shoot2.wav";
  Mix_Chunk* sfx_shoot_player = Mix_LoadWAV(soundPath.c_str());
  if (!sfx_shoot_player) {
    std::cerr << "Error: Mix_LoadWAV() failed: " << Mix_GetError() << std::endl;
    return 1;
  }
  Mix_Chunk* sfx_shoot_enemy = Mix_LoadWAV("/res/shoot1.wav");
  if (sfx_shoot_enemy == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Mix_LoadWAV error: %s", Mix_GetError());
    return -1;
  }
  Mix_Chunk* sfx_hit_player = Mix_LoadWAV("/res/hit1.wav");
  if (sfx_hit_player == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Mix_LoadWAV error: %s", Mix_GetError());
    return -1;
  }
  Mix_Chunk* sfx_hit_enemy = Mix_LoadWAV("/res/hit2.wav");
  if (sfx_hit_enemy == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Mix_LoadWAV error: %s", Mix_GetError());
    return -1;
  }
  Mix_Chunk* sfx_explosion_player = Mix_LoadWAV("/res/explosion1.wav");
  if (sfx_explosion_player == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Mix_LoadWAV error: %s", Mix_GetError());
    return -1;
  }
  Mix_Chunk* sfx_explosion_enemy = Mix_LoadWAV("/res/explosion2.wav");
  if (sfx_explosion_enemy == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Mix_LoadWAV error: %s", Mix_GetError());
    return -1;
  }
  Mix_Chunk* sfx_win = Mix_LoadWAV("/res/win.wav");
  if (sfx_win == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Mix_LoadWAV error: %s", Mix_GetError());
    return -1;
  }
  ...
  fontPath += "res/orange-kid.regular.ttf";
  ...
  ais[enemy1] = {&positions[playerEntity], 150.0f, (float)(display_width * 0.7), 0, 2.5, 0, 0.5, 0, 0.15};
